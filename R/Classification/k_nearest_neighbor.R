...
setwd(paste("/Users/kevinegan/Documents/Udemy Courses/",
            ...
            "Part 3 - Classification/Section 15 - K-Nearest Neighbors (K-NN)/R", sep = ""))
...
library(class)
y_pred <- knn(
  train = dataset_train[, 1:(ncol(dataset) - 1)],
  test = dataset_test[, 1:(ncol(dataset) - 1)],
  cl = dataset_train$Purchased,
  k = 5
)
...
# Replace predict function with KNN
y_grid <- knn(
  train = dataset_train[, 1:(ncol(dataset) - 1)],
  test = grid_set,
  cl = dataset_train$Purchased,
